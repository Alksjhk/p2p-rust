...
impl StreamManager {
    ...
    pub fn accept_syn(&mut self, proposed_id: u16, seq: u16, now: Instant) -> u16 {
    ...
    pub fn next_send_seq(&mut self, id: u16) -> Option<u16> {
    ...
    pub fn record_send(&mut self, id: u16, seq: u16, payload: Vec<u8>, now: Instant) {
        if let Some(s) = self.streams.get_mut(&id) {
            s.unacked.push(UnackedPacket { seq, payload, sent_at: now });
            s.last_activity = now;
        }
    }

    pub fn on_ack(&mut self, sid: u16, ack_num: u16) {
        if let Some(s) = self.streams.get_mut(&sid) {
            s.unacked.retain(|p| seq_gt(p.seq, ack_num));
            s.rto = INITIAL_RTO;
            s.retransmit_count = 0;
        }
    }

    pub fn on_data(&mut self, sid: u16, seq: u16) -> u16 {
        if let Some(s) = self.streams.get_mut(&sid) {
            if seq == s.recv_seq {
                s.recv_seq = s.recv_seq.wrapping_add(1);
            }
            s.last_activity = Instant::now();
        }
        self.streams.get(&sid).map(|s| s.recv_seq.wrapping_sub(1)).unwrap_or(0)
    }

    pub fn current_ack(&self, sid: u16) -> u16 {
        self.streams.get(&sid).map(|s| s.recv_seq.wrapping_sub(1)).unwrap_or(0)
    }
    ...
    pub fn retransmit_due(&mut self, now: Instant) -> Vec<(u16, Packet)> {
        let mut result = vec![];
        let mut dead = vec![];
        // Pre-compute ack numbers to avoid borrow conflicts.
        let acks: Vec<(u16, u16)> = self
            .streams
            .keys()
            .map(|&sid| (sid, self.current_ack(sid)))
            .collect();
        for (&sid, s) in &mut self.streams {
            let ack_num = acks.iter().find(|&&(id, _)| id == sid).map(|&(_, a)| a).unwrap_or(0);
            let mut i = 0;
